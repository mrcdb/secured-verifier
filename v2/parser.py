...
def parse_IMA_Image(hash_str, type_str, image_blob, containerCheckAnalysis):
    ...
    #if check-cont analysis is indicated and IMA template is not ima-cont-id, then it raise an exception
    if not type_str == 'ima-cont-id' and containerCheckAnalysis.doCheckContAnalysis:
            ...
            if field_id == 'd-ng':
                ...
                if field_id in ['hook-id', 'hook-mask']:
                    ...
                    if containerCheckAnalysis.doCheckContAnalysis:
                        if '0:' not in field and field not in containerCheckAnalysis.containers.keys():
                            print "Discarding measure relative to a stopped container"
                            return
                        check_measure = False
                        if field.startswith('0:') or containerCheckAnalysis.containers.get(field) == 'host':
                            check_measure = True
                        else:
                            for item in containerCheckAnalysis.checked_containers.split('+'):
                                if containerCheckAnalysis.containers[field] == item:
                                    check_measure = True
                                    break
                        if not check_measure:
                            print "Discarding measure relative to container ", containerCheckAnalysis.containers.get(field)
                            return
                elif field_id in ['subj', 'obj', 'bprm-subj']:
                    field = field[:-2]
...
class IMAMeasureHandler(xml.sax.ContentHandler):
    ...
    def characters(self, content):
        ...
        if self.containerId == '' and not self.hostElement:
            ...
            line = parse_IMA_Image(base64.decodestring(self.content), self.type_str, base64.decodestring(self.image_blob), self.containerCheckAnalysis)
            self.content = ''
...
class XMLParser(object):
    def parse_report_pyxb(self, report_xml, containerCheckAnalysis):
        ...
        ima_snap = [snap for snap in report.SnapshotCollection if snap.ComponentID.Id.split('_')[1] == '10'][0]

        for v in ima_snap.Values:
            item = v.orderedContent()[0].value.Objects[0]
            line = parse_IMA_Image(item.Hash[0].value(), item.Type, item.Image, containerCheckAnalysis)

    def __init__(self, report_xml, containerCheckAnalysis):
        xml.sax.parseString(report_xml, IMAMeasureHandler(containerCheckAnalysis))
...
class ASCIIParser(object):
    def __init__(self, report_ascii, containerCheckAnalysis):
        for file_line in report_ascii.split('\n'):
            ...
            if containerCheckAnalysis.doCheckContAnalysis and not ima_template == 'ima-cont-id':
                raise Exception('Invalid IMA template for making cont-check analysis')

            if ima_template == 'ima-cont-id':
                # If check-cont analysis is not indicated, it directly insert the IMA measure
                if not containerCheckAnalysis.doCheckContAnalysis or 'boot_aggregate' in file_line:
                    if 'boot_aggregate' in file_line:
                        line = file_line.split(' ')[0] + " " + file_line.split(' ')[1] + " ima-ng " + file_line.split(' ')[3] + " " + file_line.split(' ')[4]
                    else:
                        line = file_line.split(' ')[0] + " " + file_line.split(' ')[1] + " ima-ng " + file_line.split(' ')[4] + " " + file_line.split(' ')[5]
                    IMARecord(line)
                    continue
                
                self.dev_id = file_line.split(' ')[3]
                self.check_measure = False
                if '0:' not in self.dev_id and self.dev_id not in containerCheckAnalysis.containers.keys():
                    print "Discarding measure relative to a stopped container"
                    continue
                if self.dev_id.startswith('0:') or containerCheckAnalysis.containers.get(self.dev_id) == 'host':
                    self.check_measure = True
                else:
                    for item in containerCheckAnalysis.checked_containers.split('+'):
                        if containerCheckAnalysis.containers[self.dev_id] == item:
                            self.check_measure = True
                            break
                if not self.check_measure:
                    print "Discarding measure relative to container ", containerCheckAnalysis.containers[self.dev_id]
                    continue
                else:
                    line = file_line.split(' ')[0] + " " + file_line.split(' ')[1] + " ima-ng " + file_line.split(' ')[4] + " " + file_line.split(' ')[5]
                    IMARecord(line)
